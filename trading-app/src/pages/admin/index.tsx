...
const UsersPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get<User[]>('https://116.203.108.180:5000/api/auth/users', {
          headers: {
  ...
  const fetchAutoMode = async (userId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`https://116.203.108.180:5000/api/trade/automode/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleUpdateUser = async (updatedUser: User) => {
    try {
      ...
      const response = await axios.put(
        `https://116.203.108.180:5000/api/admin/users/${userId}`,
        updatedUser,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://116.203.108.180:5000/api/auth/users/${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleToggleTradeMode = async (userId: string, mode: string) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `https://116.203.108.180:5000/api/trade/automode/${userId}`,
        { autoMode: mode },
        {
          headers: {
            Authorization: `Bearer ${token}`,
