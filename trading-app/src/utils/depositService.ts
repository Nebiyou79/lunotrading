...
export const getDepositAddresses = async (token: string) => {
    const response = await axios.get('https://lunotrading.com/api/deposit/addresses', {
      headers: {
  ...
  export const updateDepositAddresses= async (data: DepositAddresses, token: string) => {
     try {
       ...
       const response = await axios.put('https://lunotrading.com/api/deposit/addresses', data, {
         headers: {
           Authorization: `Bearer ${token}`,
         },
       });
